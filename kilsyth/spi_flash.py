...
class SpiFlashDualQuad(SpiFlashCommon, AutoCSR):
    def __init__(self, pads, dummy=15, div=2, with_bitbang=True, endianness="big"):
        ...
        # 1. Enable volatile write. Toggle CS.
        # 2. Write status register to enable QPI. Toggle CS.
        # 3. Read
        ...
        wev_cmd_params = {
            4: (_format_cmd(_WEV, 4), 4*8),
            2: (_format_cmd(_WEV, 2), 2*8),
            1: (_format_cmd(_WEV, 1), 1*8)
        }
        wev_cmd, wev_cmd_width = wev_cmd_params[spi_width]

        # Write to status registers 8-15
        wsr2_cmd_params = {
            4: (_format_cmd(_WSR2, 4), 4*8),
            2: (_format_cmd(_WSR2, 2), 2*8),
            1: (_format_cmd(_WSR2, 1), 1*8)
        }
        wsr2_cmd, wsr2_cmd_width = wsr2_cmd_params[spi_width]

        # Set Quad Enable in S9
        _S9 = 0b00000010
        wsr2_arg_params = {
            4: (_format_cmd(_S9, 4), 4*8),
            2: (_format_cmd(_S9, 2), 2*8),
            1: (_format_cmd(_S9, 1), 1*8)
        }
        wsr2_arg, wsr2_arg_width = wsr2_arg_params[spi_width]
        ...
        enable_quad = [
            # 1. Enable status register volatile write 
            (wev_cmd_width//spi_width*div,
                [dq_oe.eq(1), cs_n.eq(0), sr[-wev_cmd_width:].eq(wev_cmd)]),
            (div,
                [cs_n.eq(1)]),

            # 2. Write SR to enable D/QPI
            (wsr2_cmd_width//spi_width*div,
                [dq_oe.eq(1), cs_n.eq(0), sr[-wsr2_cmd_width:].eq(wsr2_cmd)]),
            (wsr2_arg_width//spi_width*div,
                [dq_oe.eq(1), sr[-wsr2_arg_width:].eq(wsr2_arg)]),
            (div,
                [cs_n.eq(1), quad_enabled.eq(1)]),
        ]
        ...
        seq = [
            ...
            (m_cmd_width//spi_width*div,
                [sr[-m_cmd_width:].eq(m_cmd)]),
        ...
        self.sync += timeline(bus.cyc & bus.stb & quad_enabled  & (i == div - 1), tseq)
        self.sync += timeline(bus.cyc & bus.stb & ~quad_enabled & (i == div - 1), tseq_enable_quad)
...
def SpiFlashEx(pads, *args, **kwargs):
